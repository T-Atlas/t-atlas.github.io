...
页面与导航共用连续伸缩、最大 1280px 的容器，避免在 1280px 断点突然改变栏宽。桌面个人资料侧栏与正文按同一容器计算宽度，使用 sticky 跟随滚动，并按导航实际高度预留顶部空间；较矮窗口下侧栏自身可滚动，长链接名称可换行。页面采用纵向弹性布局，由主内容区域吸收短页面的剩余高度，使紧凑页脚落在视口底部。打印时恢复普通文档流。布局变更需复查 925px、1024px、1280px 附近，以及 Retina / 4K 缩放和手机宽度。
